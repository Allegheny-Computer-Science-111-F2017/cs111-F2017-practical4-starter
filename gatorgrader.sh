...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfour/ \
                                     --checkfiles MadLibs.java --singlecomments 4 --multicomments 2 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: the "new Date()" fragment exists in the code at least once
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfour \
                                     --checkfiles MadLibs.java --fragments "new Date()" --fragmentcounts 1
  determine_exit_code $?
  echo ""
